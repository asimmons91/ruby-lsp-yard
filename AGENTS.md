...
## What this is
Ruby LSP add-on (working name `ruby-lsp-yard`) that reads YARD `@param`/`@return` tags as type annotations for completion, hover, signature help and definition. Milestones M0–M5 are implemented: the add-on activates through `lib/ruby_lsp/ruby_lsp_yard/addon.rb` (gated by `depend_on_ruby_lsp!("~> 0.26.0")`), all host-indexer access goes through `lib/ruby_lsp_yard/indexer/` (adapter interface + `RubyIndexerAdapter`), YARD parsing and the signature store live in `lib/ruby_lsp_yard/documentation/` and `signature_store.rb`, the M2 inference engine in `lib/ruby_lsp_yard/inference/`, the M3 RBS bridge and gem cache in `lib/ruby_lsp_yard/rbs/` and `lib/ruby_lsp_yard/gems/`, the hover/completion/definition listeners in `lib/ruby_lsp_yard/listeners/`, the M4 comment authoring patch, context detection, completion, type lookup and skeletons in `lib/ruby_lsp_yard/authoring/`, and the M5 diagnostics linter, scanner and rules in `lib/ruby_lsp_yard/diagnostics/`. M6–M7 (Rubydex, macros) are not implemented yet.

`docs/requirements_v1.md` is the accepted V1 spec (architecture, milestones M0–M7, open decisions D1–D14) and the source of truth for scope and design. Read it before implementing anything. Key decisions already made: `yard` gem is a runtime dependency (parsing only, no Registry); all indexer access goes through an adapter because 0.26 has `RubyIndexer` and 0.27 has Rubydex; completion inside comments requires a version-guarded monkeypatch (D3); where RBS and YARD both describe a method, RBS wins. M3's implementation notes live in §8.1, M4's in §9.1 and M5's in §10.1.
