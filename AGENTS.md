...
## What this is
Ruby LSP add-on (working name `ruby-lsp-yard`) that reads YARD `@param`/`@return` tags as type annotations for completion, hover, signature help and definition. Milestone M0 is implemented: the add-on activates through `lib/ruby_lsp/ruby_lsp_yard/addon.rb` (gated by `depend_on_ruby_lsp!("~> 0.26.0")`), all host-indexer access goes through `lib/ruby_lsp_yard/indexer/` (adapter interface + `RubyIndexerAdapter`), settings and logging plumbing live in `lib/ruby_lsp_yard/`, and watched-file changes are forwarded to adapter subscribers. M1–M7 (YARD parsing, signature store, editor features) are not implemented yet.
