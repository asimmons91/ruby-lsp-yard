  ...
  module Yard
    module Listeners
      ...
      class Completion
        ...
        def initialize(response_builder, node_context, dispatcher, adapter:, store:, inference:, macros: nil,
          domains: nil, log: nil)
        ...
        def on_call_node_enter(node)
          ...
          # FR-M7-02: an implicit-`self` call has no receiver to resolve, but the enclosing namespace may bind DSL
          # domains whose methods should be offered.
          return emit_implicit_domain_items(prefix, range) if receiver.nil?
          ...
          if receiver.is_a?(Prism::SelfNode)
            items += domain_candidates(prefix, range).reject do |item|
              items.any? { |existing| existing.label == item.label }
            end
          end
        ...
        def emit_implicit_domain_items(prefix, range)
          items = domain_candidates(prefix, range)
          return if items.empty?
          ...
          @log&.error("Domain completion failed: #{e.class}: #{e.message}")
        end

        # FR-M7-02: completions contributed by the `@!domain` (and `.solargraph.yml`) domains bound to the enclosing
        # namespace.
        def domain_candidates(prefix, range)
          return [] unless @domains

          owner = enclosing_class
          return [] unless owner

          descriptors = @domains.domains_for(owner)
          return [] if descriptors.empty?

          members = descriptors.map do |descriptor|
            Inference::Resolution::Member.new(descriptor[:owner], descriptor[:singleton], descriptor[:owner], nil)
          end
          resolution = Inference::Resolution.new(members: members)
          member_items(resolution, prefix, range, internal: true)
        end
        ...
        def collect_entries(resolution, prefix, internal)
          ...
          resolution.members.each_with_index do |member, member_index|
            ...
            macro_candidates(member, prefix).each do |name, signature|
              next if name.empty? || name.end_with?("=")
              next unless visible?(signature.visibility, internal, member)

              entry = entries[name] ||= {
                definition: macro_definition(name, member, signature),
                member: member,
        ...
        # FR-M7-01: methods generated by attached macros at DSL call sites, which the host index does not know
        # about. Ancestors contribute their generated methods too, mirroring ordinary inheritance.
        def macro_candidates(member, prefix)
          return {} unless @macros

          entries = {}
          ancestors_for(member.owner).uniq.each do |owner|
            @macros.entries_for(owner, singleton: member.singleton).each do |name, signature|
        ...
        def macro_definition(name, member, signature)
          Indexer::Definition.new(
            name: name,
            owner: signature.owner || member.owner,
            kind: :method,
            visibility: signature.visibility,
            uri: signature.uri,
            location: signature.location,
            full_location: signature.location,
            file_name: signature.uri ? File.basename(signature.uri.path.to_s) : nil,
            comments: nil,
            parameters: []
          )
        end
        ...
        def signature_for(entry)
          return entry[:signature].with_type_bindings(type_bindings(entry[:member], entry[:signature])) if entry[:signature]
